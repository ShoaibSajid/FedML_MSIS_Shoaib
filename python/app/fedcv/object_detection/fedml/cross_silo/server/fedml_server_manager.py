...
class FedMLServerManager(FedMLCommManager):
    ...
    def handle_message_receive_model_from_client(self, msg_params):
        ...
        self.aggregator.add_local_trained_result(self.client_real_ids.index(sender_id), model_params, local_sample_number)

        try:
            mAPs = msg_params.get('mAPs')
        except Exception as e:
            mAPs = []
            print(f"\n\n\n\t\tSome error happened {e}.\n\t\t No mAP received from client {sender_id} side.\n\n\n")
        try:
            self.aggregator.add_local_trained_mAPs(self.client_real_ids.index(sender_id), mAPs)
        except:
            print('Some Problem')
        ...
        if b_all_received:
            ...
            global_model_params = self.aggregator.aggregate_and_compare()
            ...
            self.client_id_list_in_this_round = self.aggregator.client_selection(self.args.round_idx, self.client_real_ids, self.args.client_num_per_round)
            self.data_silo_index_list = self.aggregator.data_silo_selection(self.args.round_idx, self.args.client_num_in_total, len(self.client_id_list_in_this_round),)
            ...
            for receiver_id in self.client_id_list_in_this_round:
                ...
                if type(global_model_params) is dict:
                    self.send_message_sync_model_to_client(receiver_id, global_model_params[client_index], client_index,)
                else:
            ...
            del(self.aggregator.mAPs)
            self.aggregator.mAPs=dict()
